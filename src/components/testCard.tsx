...
import React, { useContext, useState } from "react";
import { ViewMetaData } from "./dataController";
import { FiEye, FiEyeOff, FiTrash2, FiEdit, FiCheck } from "react-icons/fi";
import { AppContext } from "../context/AppContext";
...
const AttributeCard: React.FC<AttributeCardProps> = ({ data }) => {
    ...
    const [editedName, setEditedName] = useState(data.testObject._NAME_);

    const toggleRaw = () => setShowRaw((prev) => !prev);
    const toggleEdit = () => setIsEditing((prev) => !prev);

    const { setOriginalValidations } = useContext(AppContext);
    
    const handleSaveEdit = () => {
        setOriginalValidations((or) => {
            if (!or) return undefined;
            const updatedValidations = { ...or, _TESTS_: { ...or._TESTS_ } };
            const testArray = updatedValidations._TESTS_[data.api] || [];
            const testIndex = testArray.findIndex(t => t._NAME_ === data.testObject._NAME_);
            
            if (testIndex !== -1) {
                testArray[testIndex] = { ...testArray[testIndex], _NAME_: editedName };
            }
    ...
    const bgClasses = data.void ? "bg-gray-100" : data.matched ? "bg-green-100" : "bg-red-100";

    return (
        <div className={`relative rounded-lg shadow-md p-6 flex flex-col justify-between w-full max-w-md transition-transform hover:shadow-lg ${bgClasses}`}>
            <div className="absolute top-4 right-4 flex gap-2">
                <button onClick={toggleRaw} className="text-gray-500 hover:text-gray-700 focus:outline-none" title={showRaw ? "Hide Raw" : "View Raw"}>
                    {showRaw ? <FiEyeOff size={20} /> : <FiEye size={20} />}
                </button>
                <button onClick={toggleEdit} className="text-blue-500 hover:text-blue-700 focus:outline-none" title="Edit">
                    {isEditing ? <FiCheck size={20} onClick={handleSaveEdit} /> : <FiEdit size={20} />}
                </button>
                <button onClick={fundelete} className="text-red-500 hover:text-red-700 focus:outline-none" title="Delete">
                    <FiTrash2 size={20} />
                </button>
            </div>
            <div>
                {isEditing ? (
                    ...
                    <h3 className="text-xl font-semibold mb-2 text-gray-800">{data.testObject._NAME_}</h3>
                )}
                {data.void ? (
                    <p className="text-sm text-gray-500">Not Present Here</p>
                ) : (
                    <p className={`text-sm ${data.notIdenticalError ? "text-red-600" : "text-blue-600"}`}>
                        {data.notIdenticalError ? "Not Identical" : "Tests are identical"}
                    </p>
                )}
            </div>
